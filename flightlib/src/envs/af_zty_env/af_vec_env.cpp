...
AgileFlightVecEnv<EnvBaseName>::AgileFlightVecEnv() {
...
template<typename EnvBaseName>
AgileFlightVecEnv<EnvBaseName>::AgileFlightVecEnv(const std::string& cfg,
...
template<typename EnvBaseName>
AgileFlightVecEnv<EnvBaseName>::AgileFlightVecEnv(const YAML::Node& cfg_node) {
  cfg_ = cfg_node;
  configEnv(cfg_);
  random_reset_ = true;
  std::cout << "Created Vecterized Env:" << cfg_<< std::endl;
}

template<typename EnvBaseName>
bool AgileFlightVecEnv<EnvBaseName>::reset(Ref<MatrixRowMajor<>> obs) {
...
template<typename EnvBaseName>
bool AgileFlightVecEnv<EnvBaseName>::reset(Ref<MatrixRowMajor<>> obs, bool random) {
  if (obs.rows() != this->num_envs_ || obs.cols() != this->obs_dim_) {
    this->logger_.error(
      "Input matrix dimensions do not match with that of the environment.");
    return false;
...
template<typename EnvBaseName>
bool AgileFlightVecEnv<EnvBaseName>::step(Ref<MatrixRowMajor<>> act,
...
template<typename EnvBaseName>
void AgileFlightVecEnv<EnvBaseName>::perAgentStep(int agent_id,
...
template<typename EnvBaseName>
bool AgileFlightVecEnv<EnvBaseName>::getQuadAct(Ref<MatrixRowMajor<>> quadact) {
  bool valid = true;
  for (int i = 0; i < this->num_envs_; i++) {
...
template<typename EnvBaseName>
bool AgileFlightVecEnv<EnvBaseName>::getQuadState(Ref<MatrixRowMajor<>> quadstate) {
  bool valid = true;
  for (int i = 0; i < this->num_envs_; i++) {
...
template<typename EnvBaseName>
bool AgileFlightVecEnv<EnvBaseName>::setCollisionStopNum(int num) {
  bool valid = true;
  for (int i = 0; i < this->num_envs_; i++) {
    valid &= this->envs_[i]->setCollisionStopNum(num);
  }
  return valid;
}
...
template class AgileFlightVecEnv<AgileFlightEnv>;
