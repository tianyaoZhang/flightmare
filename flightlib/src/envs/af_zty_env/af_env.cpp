...
AgileFlightEnv::AgileFlightEnv()
  : AgileFlightEnv(getenv("FLIGHTMARE_PATH") +
                std::string("/flightpy/configs/vision/af_config.yaml"),
              0) {}

AgileFlightEnv::AgileFlightEnv(const std::string &cfg_path, const int env_id)
  : EnvBase() {
...
AgileFlightEnv::AgileFlightEnv(const YAML::Node &cfg_node, const int env_id) : EnvBase() {
  cfg_ = cfg_node;

  //
  init();
  env_id_ = env_id;
  // logger_.info("Configuration:", cfg_node);
}

void AgileFlightEnv::init() {
  //
  is_collision_ = false;
  ...
  obs_dim_ = afenv::kNObs;
  act_dim_ = afenv::kNAct;
  rew_dim_ = 0;
  num_detected_obstacles_ = afenv::kNObstacles;
...
bool AgileFlightEnv::reset(Ref<Vector<>> obs) {
  quad_state_.setZero();
  pi_act_.setZero();
  old_pi_act_.setZero();
  is_collision_ = false;
...
bool AgileFlightEnv::reset(Ref<Vector<>> obs, bool random) { return reset(obs); }

bool AgileFlightEnv::getObs(Ref<Vector<>> obs) {
  ...
  // Vector<4> att = quad_state_.q()
  // compute rotation matrix
  // Vector<9> ori = Map<Vector<>>(quad_state_.R().data(), quad_state_.R().size());

  // get N most closest obstacles as the observation
  Vector<afenv::kNObstacles * afenv::kNObstaclesState> obstacle_obs;
  getObstacleState(obstacle_obs);

  // Observations
  // obs << goal_linear_vel_, ori, quad_state_.v, obstacle_obs;
  obs << quad_state_.p ,quad_state_.qx, quad_state_.v, quad_state_.w, obstacle_obs;   // 3+4+3+3+ 4*10 = 53
  return true;
}

bool AgileFlightEnv::getObstacleState(Ref<Vector<>> obs_state) {
  ...
  for (size_t sort_idx : sort_indexes(relative_pos_norm_)) {
    if (idx >= afenv::kNObstacles) break;
    ...
    if (idx < relative_pos.size()) {
      ...
      if (relative_pos_norm_[sort_idx] <= max_detection_range_) {
        ...
        obs_state.segment<afenv::kNObstaclesState>(
          idx * afenv::kNObstaclesState)
      ...
      obs_state.segment<afenv::kNObstaclesState>(
        idx * afenv::kNObstaclesState) =
        Vector<afenv::kNObstaclesState>(max_detection_range_,
...
bool AgileFlightEnv::step(const Ref<Vector<>> act, Ref<Vector<>> obs,
                     Ref<Vector<>> reward) {
...
bool AgileFlightEnv::simDynamicObstacles(const Scalar dt) {
...
bool AgileFlightEnv::computeReward(Ref<Vector<>> reward) {
  ...
  for (size_t sort_idx : sort_indexes(relative_pos_norm_)) {
    ...
    if (relative_pos_norm_[sort_idx] <=
        obstacle_radius_[sort_idx] + dist_margin_) {
      // compute distance penalty
      collision_penalty += collision_coeff_ * 
        std::exp(collision_exp_coeff_ * (relative_dist - obstacle_radius_[sort_idx]) );
    }
...
bool AgileFlightEnv::getQuadAct(Ref<Vector<>> act) const {
...
bool AgileFlightEnv::getQuadState(Ref<Vector<>> obs) const {
  if (quad_state_.t >= 0.0 && (obs.rows() == afenv::kNQuadState)) {
...
bool AgileFlightEnv::getDepthImage(Ref<DepthImgVector<>> depth_img) {
...
bool AgileFlightEnv::getImage(Ref<ImgVector<>> img, const bool rgb) {
  if (!rgb_camera_) {
...
bool AgileFlightEnv::loadParam(const YAML::Node &cfg) {
  if (cfg["environment"]) {
...
bool AgileFlightEnv::configDynamicObjects(const std::string &yaml_file) {
  //
...
bool AgileFlightEnv::configStaticObjects(const std::string &csv_file) {
  //
  if (!(file_exists(csv_file))) {
    logger_.error("Configuration file %s does not exists.", csv_file);
    return false;
...
bool AgileFlightEnv::configCamera(const YAML::Node &cfg) {
  if (!cfg["rgb_camera"]) {
...
bool AgileFlightEnv::addQuadrotorToUnity(const std::shared_ptr<UnityBridge> bridge) {
...
bool AgileFlightEnv::setUnity(bool render) {
...
bool AgileFlightEnv::connectUnity(void) {
...
FrameID AgileFlightEnv::updateUnity(const FrameID frame_id) {
...
void AgileFlightEnv::disconnectUnity(void) {
...
int AgileFlightEnv::getNumDetectedObstacles(void) { return num_detected_obstacles_; }

std::ostream &operator<<(std::ostream &os, const AgileFlightEnv &vision_env) {
  os.precision(3);
